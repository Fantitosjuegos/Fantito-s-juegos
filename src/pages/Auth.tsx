...
const Auth = () => {
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      toast({ title: 'Error', description: 'Could not delete account. Email privacy@fantitosjuegos.fun', variant: 'destructive' });
    } finally {
      setDeleting(false);
    ...
    return (
      <main className="relative min-h-[100dvh] max-w-[430px] mx-auto bg-background overflow-hidden flex flex-col px-5 pt-6 pb-8">
        ...
        <div className="flex flex-col items-center text-center mb-8">
          <img src={fantitoLoader} alt="Fantito" className="w-20 h-20 object-contain mb-4" />
          <h1 className="font-display font-black text-2xl text-foreground">Your account</h1>
  ...
  return (
    <main className="relative min-h-[100dvh] max-w-[430px] mx-auto bg-background overflow-hidden flex flex-col">
      ...
      <div className="pointer-events-none absolute inset-0" aria-hidden>
        ...
        {dots.map(d => (
          <span key={d.id} className="absolute rounded-full vs-float"
            style={{ left: `${d.left}%`, top: `${d.top}%`, width: d.size, height: d.size, background: 'hsl(var(--primary) / 0.55)', boxShadow: '0 0 8px hsl(var(--primary) / 0.7)', animationDelay: `${d.delay}s`, animationDuration: `${4 + (d.id % 4)}s` }} />
        ))}
      </div>
      ...
      <div className="relative z-10 flex-1 flex flex-col px-5 pt-2 pb-6">
        ...
        <div className="flex flex-col items-center text-center pt-2 pb-5">
          <div className="relative">
            ...
            <img src={fantitoLoader} alt="Fantito" className="relative w-24 h-24 object-contain vs-float" loading="eager" />
          </div>
          ...
          <h1 className="mt-1 font-display text-3xl font-black text-foreground leading-tight">
            {isSignup ? (<>Unlock your <span style={{ color: 'hsl(var(--primary))' }}>chaos universe</span></>) : (<>Continue the <span style={{ color: 'hsl(var(--primary))' }}>chaos</span></>)}
          </h1>
          <div className="mt-3 h-5 relative w-full max-w-[300px] overflow-hidden">
            {FANTITO_LINES.map((line, i) => (
              <p key={i} className="absolute inset-x-0 text-sm text-foreground/80 font-display transition-all duration-500"
                style={{ opacity: i === lineIdx ? 1 : 0, transform: `translateY(${i === lineIdx ? 0 : 6}px)` }}>
                {line}
              </p>
            ))}
          </div>
        ...
        <div className="space-y-2.5">
          ...
          {/* Apple — hidden until Apple Developer account is active */}
          {false && (
            <button type="button" onClick={() => handleOAuth('apple')} disabled={!!oauthBusy}
        ...
        {!showEmail ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-2.5 mt-5 vs-rise">
            <input type="email" value={email} onChange={e => setEmail(e.target.value)}
              placeholder="Where should Fantito send the chaos?" autoComplete="email" required
            ...
            <input type="password" value={password} onChange={e => setPassword(e.target.value)}
              placeholder="Your secret chaos access 👀" autoComplete={isSignup ? 'new-password' : 'current-password'} required
              className="w-full bg-card/60 backdrop-blur-sm border border-white/10 px-4 py-3 text-sm text-foreground placeholder:text-muted-foreground/70 focus:outline-none focus:border-primary/60 focus:ring-2 focus:ring-primary/30 rounded-xl transition-all" />
            {!isSignup && (
            ...
            <button type="submit" disabled={submitting}
              ...
              style={{ background: 'linear-gradient(120deg, hsl(var(--primary)), hsl(var(--accent)))', boxShadow: '0 0 22px -6px hsl(var(--primary) / 0.65)' }}>
        ...
        <div className="mt-auto pt-6 text-center">
          <button type="button" onClick={() => { setMode(isSignup ? 'signin' : 'signup'); setShowEmail(false); }}
            ...
            {isSignup ? (<>Already in the chaos? <span className="text-primary font-semibold">Sign in</span></>) : (<>New here? <span className="text-primary font-semibold">Build your profile</span></>)}
          </button>
        </div>
      </div>
    </main>
